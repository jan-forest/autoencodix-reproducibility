...
for b_string in beta_strings:
    ...
    df_cov_temp = pd.read_parquet(
        rootdir + "reports/"+run_id+"/" +"/latent_cov_per_epoch_"+run_id+".parquet"
    )
    ...
    df_cov_temp["beta"] = [
        beta
        * annealer(epoch_current=e, total_epoch=len(df_cov_temp.epoch), func="logistic-mid")
    ...
    df_cov_temp["Recon. Loss"] = df_loss["valid_r2"] ## Switch to R2
...
for b_index, b_string in enumerate(beta_strings):
    ...
    sns.heatmap(
        pd.DataFrame(df_cov_temp.set_index("epoch")["Recon. Loss"]).T,
        cmap="Oranges",
